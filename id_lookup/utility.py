...
def _load_config():
	...
	cfg.read(os.path.join(get_current_directory(), 'id_lookup.config'))
	return cfg
...
def _setup_logging():
	...
	if config['logging']['directory'] == '':
		return get_file_logger(os.path.join(get_current_directory(), 'id_lookup.log'),
								config['logging']['log_level'])
	else:
		return get_file_logger(config['logging']['directory'], config['logging']['log_level'])
